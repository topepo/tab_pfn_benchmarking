...
for size in size_seq:
    ...
    pred_time = {"num_train": len(tr_x), "time": pred_time, "gpu": "cuda"}
...
all_pred.to_csv("predictions_by_training_size_cuda.csv", index=False)
all_times.to_csv("predictions_by_training_size_times_cuda.csv", index=False)
